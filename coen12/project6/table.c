...
/*
	...
	Description: Sorts the values around the pivot and 
*/
...
static int partition(void **elts, int lo, int hi, int (*compare)())
{
	...
	x = elts[hi];	//x is hi value
	sep = lo;		//sep is lo
	for(i = lo; i < hi; i++) {	
		if((*compare)(elts[i], x) < 0) {	//if data is less than hi
			temp = elts[i];		//swap it with sep and replace sep with value
			elts[i] = elts[sep];
			elts[sep++] = temp;
		}
	}
...
/* 
	...
	Description: Sorts the array passed by elts
*/
...
void *getElements(SET *sp)
{
	...
	quicksort(elts, 0, numElements(sp) - 1, sp->compare);	
    return elts;
}
